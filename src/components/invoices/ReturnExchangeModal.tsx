...
const ReturnExchangeModal: React.FC<ReturnExchangeModalProps> = ({
  ...
  const processReturn = async () => {
    try {
      ...
      const totalReturnQuantity = itemsToReturn.reduce((sum, item) => sum + item.returnQuantity, 0);const totalOriginalQuantity = invoiceItems.reduce((sum, item) => sum + item.quantity, 0);if (totalReturnQuantity === totalOriginalQuantity) {
