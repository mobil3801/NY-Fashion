...
const ReturnExchangeModal: React.FC<ReturnExchangeModalProps> = ({
  ...
  const processReturn = async () => {
    try {
      ...
      const totalReturnQuantity = itemsToReturn.reduce((sum, item) => sum + item.returnQuantity, 0);const totalOriginalQuantity = invoiceItems.reduce((sum, item) => sum + item.quantity, 0);if (totalReturnQuantity === totalOriginalQuantity) {await window.ezsite.apis.tableUpdate(36856, { ID: invoice.id,
            status: 'refunded',
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <Textarea id="notes" placeholder="Any additional notes about the return..." value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} />
            ...
            <CardContent>
              {loading ? <div className="text-center py-8">Loading items...</div> : <div className="border rounded-lg overflow-hidden">
                  <Table>
                    ...
                    <TableBody>
                      {invoiceItems.map((item) => {const returnItem = returnItems.find((ri) => ri.itemId === item.id);const maxReturnQty = item.quantity - (item.returnedQuantity || 0);return <TableRow key={item.id}>
                            <TableCell>
                              <div>
                            ...
                            <TableCell className="text-center">
                              <Input type="number" min="0" max={maxReturnQty} value={returnItem?.returnQuantity || 0} onChange={(e) => handleReturnQuantityChange(item.id, parseInt(e.target.value) || 0)} className="w-20 text-center" />

                            </TableCell>
                            <TableCell>
                              <Select value={returnItem?.condition || 'good'} onValueChange={(value) => handleConditionChange(item.id, value)} disabled={!returnItem?.returnQuantity}>

                                <SelectTrigger className="w-40">
                                  <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                  {itemConditions.map((condition) => <SelectItem key={condition.value} value={condition.value}>
                                      {condition.label}
                                    </SelectItem>)}
                                </SelectContent>
                              </Select>
