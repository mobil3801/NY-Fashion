  ...
  const runSingleTest = async (testName: string): Promise<void> => {
    switch (testName) {
      ...
      case 'API Endpoint':
        ...
        }break;case 'WebSocket Support':if (!('WebSocket' in window)) {throw new Error('WebSocket not supported');}break;case 'Local Storage':localStorage.setItem('diagnostics_test', 'test');
