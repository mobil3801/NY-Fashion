...
const ApiDebugDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    }, 5000);return () => clearInterval(interval);}, [isAutoRefresh]);const callStats = { total: apiCalls.length, success: apiCalls.filter((c) => c.status === 'success').length, error: apiCalls.filter((c) => c.status === 'error').length,
    pending: apiCalls.filter((c) => c.status === 'pending' || c.status === 'retrying').length
  };
