...
*/export async function testDnsResolution(domains: string[] = ['google.com', 'cloudflare.com']): Promise<{domain: string;resolved: boolean;duration: number;}[]> {const results = await Promise.allSettled(domains.map(async (domain) => {
    const startTime = performance.now();
    try {
