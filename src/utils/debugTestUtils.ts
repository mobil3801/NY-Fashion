...
: Promise<BenchmarkResult> {
  ...
  try {
    ...
    if (uploadTest) {
      ...
      }}} catch (error) {console.warn('Bandwidth test failed:', error);}const result: BenchmarkResult = { latency, bandwidth, reliability };console.log('📊 Benchmark completed:', result);
  return result;
}
 ...
 */
export async function testDnsResolution(domains: string[] = ['google.com', 'cloudflare.com']): Promise<{
  domain: string;
  resolved: boolean;
  duration: number;
}[]> {
