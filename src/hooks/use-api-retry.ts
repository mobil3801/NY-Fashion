...
export function useApiRetry() {
  ...
  useEffect(() => {isMountedRef.current = true;return () => {isMountedRef.current = false; // Abort all ongoing operations
      ...
      let debugCallId: string | null = null;if (debugContext?.addApiCall) {debugCallId = debugContext.addApiCall({ timestamp: new Date(), operation: options?.operation || 'unknown', method: options?.method || 'GET', url: options?.url || 'unknown', attempt: 1, duration: null, status: 'pending' });}try {const result = await executeWithRetry(fn, {
