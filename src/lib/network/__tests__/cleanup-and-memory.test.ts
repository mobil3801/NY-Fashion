    ...
    it('should not notify listeners after destroy', async () => {
      ...
      try {
        ...
        // Expected to fail after destroy
          ...
          if (global.gc) {global.gc();} // Verify no lingering effects
          clients.forEach((c) => {expect(() => c.get('/test')).not.toThrow(); // The requests may return rejected promises, but shouldn't cause errors
            });
        });
